...
/*
기능 : 함수를 통해 필요한 파일들의 세부 내용을 출력하고 구조체에 file 정보 저장하는 함수
*/
int file_index=0;
char space[256];
...
void print(char *file,char *filename, int filesize,
           int  mode, int blocksize, int links,int uid,int gid, int access,int change,int modify)
{
  // 시간을 우리가 보기쉽게 볼 수 있는 tm 구조체 구성 -> t=localtime(시간) 를 넣어서 나온 구조체 활용예정
    struct tm *t; 
    space[file_index]=filesize;

    if(file_index==0){                          //구조체 초기화 fr : find_result 구조체 초기화
    ...
    if((!strcmp(file,filename)) && space[0] == space[file_index]){        //어떤파일 넣고 출력할건지 결정 조건문
        char *cwd_file = getcwd(NULL,2560);                               //현재 경로 출력
        ...
        //sprintf(fr[file_index].cwd, "%s/%s", getcwd(NULL,NAMMAXLEN), fr[file_index].filename); //아래 과정 통합
        strcat(fr[file_index].cwd,"/");
        strcat(fr[file_index].cwd,fr[file_index].filename);               //경로를 절대경로로 합쳐주는 과정
        
    //지금까지 넣은 file_index가 같은 폴더내에 있는지 확인 -> 
        // !strcmp(fr[0].cwd,fr[i].cwd)!=0 구문이상함. -> strcmp(fr[0].cwd,fr[i].cwd)==0
...
int dir_plus(char *wd, int depth){   
    ...
    for(int i=0; i<scan; i++){                          //scan으로 받은 .경로에 있는 파일과 폴더 개수만큼
        ...
        // strcmp는 == 0으로 비교해야되는데 무언가 실수 하신듯함
        /* strcmp(namelist[i]->d_name, "."))==0|| (strcmp(namelist[i]->d_name,"..")==0*/
        ...
        if(((fstat.st_mode & S_IFDIR) != S_IFDIR && (fstat.st_mode & S_IFMT) != S_IFDIR  )) {
          dir_size+=fstat.st_size; // 해당 디렉토리 안의 내부 파일 size
        }
